"""
Evaluate SUMO's default car-following and lane-changing behavior.

This baseline does not load an RL checkpoint and does not send TraCI
speed/lane-change commands to CAVs. Metrics are exported for mixed traffic,
CAVs, and HDVs so the cohorts match the RL-control evaluation.
"""
...
def evaluate_baseline(cfg):
    ...
    cfg.exp_name = cfg.get("exp_name", "sumo_baseline")
    cfg = parse_cfg(cfg)
    set_seed(cfg.seed)
    ...
    print(colored('Baseline: SUMO default car-following + lane-changing', 'blue', attrs=['bold']))
    print(colored(
