...
class SumoIntersectionRWEnv(gym.Env):
    """
    ...
    def __init__(
        ...
        allowed_control_lanes: Optional[object] = None,
        ...
        self.sumo_gui_delay_ms = float(sumo_gui_delay_ms)
        ...
        self._allowed_control_lanes = self._parse_allowed_control_lanes(allowed_control_lanes)
        ...
        self._max_energy = self._get_energy(_MAX_SPEED, _MAX_ACCEL)
    ...
    def _start_sumo(self):
        ...
        if self.use_gui and self.sumo_gui_delay_ms > 0:
            sumo_cmd += ["--delay", str(self.sumo_gui_delay_ms)]
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            sumo_port = sock.getsockname()[1]

        label = f"cav_env_{id(self)}"
        traci.start(sumo_cmd, port=sumo_port, label=label)
    ...
    @staticmethod
    def _parse_allowed_control_lanes(allowed_control_lanes) -> Dict[str, List[int]]:
        """Parse optional per-edge lane whitelist for OSM through-lane control.

        String format:
            edgeA:0|1|2;edgeB:1|2|3
        Ranges such as edgeB:1-3 are also accepted.
        """
        if allowed_control_lanes in (None, "", {}, []):
            return {}
        if isinstance(allowed_control_lanes, dict):
            items = allowed_control_lanes.items()
        else:
            items = []
            for raw_entry in str(allowed_control_lanes).split(";"):
                ...
                if sep not in entry:
                    raise ValueError(
                        "allowed_control_lanes entries must use edge:lanes, "
                        f"got {entry!r}"
                    )
                edge, lanes = entry.split(sep, 1)
                items.append((edge.strip(), lanes.strip()))

        parsed: Dict[str, List[int]] = {}
        for edge, lanes in items:
            lane_values: List[int] = []
            if isinstance(lanes, (list, tuple, set)):
                lane_tokens = [str(v) for v in lanes]
            else:
                lane_tokens = str(lanes).replace("|", " ").replace(",", " ").split()
            for token in lane_tokens:
                if "-" in token:
                    lo_s, hi_s = token.split("-", 1)
                    lo, hi = int(lo_s), int(hi_s)
                    step = 1 if hi >= lo else -1
                    lane_values.extend(range(lo, hi + step, step))
                else:
                    lane_values.append(int(token))
            unique_sorted = sorted(set(lane_values))
            if unique_sorted:
                parsed[str(edge)] = unique_sorted
        return parsed

    def _logical_lane_count(self, edge_id: str) -> int:
        allowed = self._allowed_control_lanes.get(edge_id)
        return min(len(allowed), 3) if allowed else 3

    def _logical_lane_idx(self, edge_id: str, physical_lane_idx: int) -> int:
        allowed = self._allowed_control_lanes.get(edge_id)
        if not allowed:
            return int(np.clip(physical_lane_idx, 0, 2))
        if physical_lane_idx in allowed:
            logical_idx = allowed.index(physical_lane_idx)
        else:
            logical_idx = min(
                range(len(allowed)),
                key=lambda idx: abs(allowed[idx] - physical_lane_idx),
            )
        return int(np.clip(logical_idx, 0, 2))

    def _target_lane_idx(self, edge_id: str, physical_lane_idx: int, lc_cmd: float) -> int:
        allowed = self._allowed_control_lanes.get(edge_id)
        if not allowed:
            if lc_cmd > _LANE_CHANGE_THRESH and physical_lane_idx < 2:
                return physical_lane_idx + 1
            if lc_cmd < -_LANE_CHANGE_THRESH and physical_lane_idx > 0:
                return physical_lane_idx - 1
            return physical_lane_idx

        # If a vehicle is already in a turn-only lane, pull it back toward the
        # nearest allowed through lane regardless of the current policy command.
        if physical_lane_idx not in allowed:
            return min(allowed, key=lambda lane: abs(lane - physical_lane_idx))

        logical_idx = allowed.index(physical_lane_idx)
        if lc_cmd > _LANE_CHANGE_THRESH and logical_idx < len(allowed) - 1:
            return allowed[logical_idx + 1]
        if lc_cmd < -_LANE_CHANGE_THRESH and logical_idx > 0:
            return allowed[logical_idx - 1]
        return physical_lane_idx
    ...
    def _refresh_entry_steps(self):
        ...
        for cav_id in current:
            if cav_id not in self._cav_entry_time:
                ...
                try:
                    ...
                    self._cav_entry_speed[cav_id] = _MAX_SPEED
        ...
        for c in gone:
            ...
            entry_speed = max(self._cav_entry_speed.get(c, _MAX_SPEED), 0.0)
            ...
            t_free = (controlled_distance - 5.1) / _MAX_SPEED + \
                    ((_MAX_SPEED - entry_speed) ** 2) / (2 * _MAX_ACCEL * _MAX_SPEED)
            ...
            # r_term_time = _W_TERMINAL_TIME * ((controlled_distance - 5.1) / (actual_time * _MAX_SPEED)) ** 2
            ...
            try:
                ...
                exit_speed = 0.0
            r_term_speed = _W_TERMINAL_SPEED * (exit_speed / _MAX_SPEED) ** 2
    ...
    def _get_single_cav_obs(self, veh_id: str) -> np.ndarray:
        ...
        speed = self._veh_speed(veh_id)
        accel = self._veh_accel(veh_id)
        physical_lane_idx = self._veh_lane_idx(veh_id)
        edge_id = self._veh_road_id(veh_id)
        lane_idx = self._logical_lane_idx(edge_id, physical_lane_idx)
        lane_count = self._logical_lane_count(edge_id)
        lane_id = self._veh_lane_id(veh_id)
        lane_pos = self._veh_lane_pos(veh_id)
        lane_length = self._lane_length(lane_id)
        current_leader = self._get_neighbor(veh_id, "leader", 0)

        obs[0] = speed / _MAX_SPEED
        ...
        obs[5] = lane_pos / lane_length      # edge length normalization
            ...
            for lane_offset in [0, 1, -1]:  # current, left, right
                ...
                if target_lane < 0 or target_lane >= lane_count:
                    ...
                    if neighbor is not None:
                        ...
                        obs[idx + 1] = (n_speed - speed) / _MAX_SPEED
                        obs[idx + 2] = 1.0
                    else:
                        obs[idx] = 1.0
                        obs[idx + 1] = 0.0
        ...
        for i, cav_id in enumerate(cav_ids):
            try:
                ...
                if self.cav_control_mode in {"hybrid", "longitudinal_only"}:
                    ...
                    new_speed = min(new_speed, _MAX_SPEED)
                ...
                if self.cav_control_mode in {"hybrid", "lane_change_only"}:
                    ...
                    edge_id = self._veh_road_id(cav_id)
                    target_lane = self._target_lane_idx(edge_id, lane_idx, lc_cmd)
                    conn.vehicle.changeLane(cav_id, target_lane, self.sumo_step)
