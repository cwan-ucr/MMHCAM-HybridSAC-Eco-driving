...
class SumoIntersectionEnv(gym.Env):
    ...
    def _get_active_cavs(self) -> List[str]:
        ...
        for veh_id in all_vehs:
            try:
                is_cav = self._conn.vehicle.getTypeID(veh_id) == "cav"
                past_entry_segment = (
                    self._conn.vehicle.getLanePosition(veh_id)
                    > self.control_start_distance_m
                )
    ...
    def _refresh_entry_steps(self):
        ...
        for veh_id in self._get_control_area_vehicles():
            if veh_id in self._trip50_tracker:
                continue
            try:
                veh_type = str(self._conn.vehicle.getTypeID(veh_id)).lower()
                veh_group = "cav" if "cav" in veh_type else "hdv"
                accel0 = float(self._conn.vehicle.getAcceleration(veh_id))
            except traci.exceptions.TraCIException:
                continue
        ...
        for cav_id in current:
            if cav_id not in self._cav_entry_time:
                ...
                try:
                    self._cav_entry_speed[cav_id] = self._conn.vehicle.getSpeed(cav_id)
                    self._cav_entry_lane_pos[cav_id] = self._conn.vehicle.getLanePosition(cav_id)
                    if self.rl_control:
        ...
        for c in gone:
            ...
            t_free = controlled_distance / _MAX_SPEED + \
            ...
            r_term_time = _W_TERMINAL_TIME * (controlled_distance / (actual_time * _MAX_SPEED)) ** 2

            # Exit speed — higher is better (agent not decelerating before exit)
            try:
                exit_speed = self._conn.vehicle.getSpeed(c)
            except traci.exceptions.TraCIException:
                exit_speed = 0.0
    ...
    def _update_trip50_metrics(self):
        ...
        for veh_id in list(self._trip50_tracker.keys()):
            ...
            if rec is None:
                continue
            try:
                speed = float(conn.vehicle.getSpeed(veh_id))
                accel = float(conn.vehicle.getAcceleration(veh_id))
                edge_id = str(conn.vehicle.getRoadID(veh_id))
                lane_pos = float(conn.vehicle.getLanePosition(veh_id))
            except traci.exceptions.TraCIException:
            ...
            leader = conn.vehicle.getLeader(veh_id, _MAX_DETECT_DIST)
            if leader is not None and leader[0] != "":
                leader_speed = conn.vehicle.getSpeed(leader[0])
    ...
    def _get_single_cav_obs(self, veh_id: str) -> np.ndarray:
        ...
        lane_pos = conn.vehicle.getLanePosition(veh_id)
        ...
        follow_mode = self._get_following_mode_onehot(veh_id)
        obs[6:9] = follow_mode
            ...
            for lane_offset in [0, 1, -1]:  # current, left, right
                ...
                if target_lane < 0 or target_lane > 2:
                    ...
                    if neighbor is not None:
                        ...
                        n_speed = conn.vehicle.getSpeed(n_id)
    ...
    def _get_following_mode_onehot(self, veh_id: str) -> np.ndarray:
        """
        ...
        try:
            lead_id, _ = leader
            lead_type = str(self._conn.vehicle.getTypeID(lead_id)).lower()
            if "cav" in lead_type:
                mode[2] = 1.0
            else:
                mode[1] = 1.0
    ...
    def _get_neighbor(
        ...
        try:
            if mode == "leader":
                if lane_offset == 0:
                    result = conn.vehicle.getLeader(veh_id, _MAX_DETECT_DIST)
                    if result is not None and result[0] != "":
                        return result  # (id, dist)
                    return None
                else:
                    ...
                    if neighbors.__len__() > 0:
                        return neighbors[0]  # Return the first neighbor found
                    return None
            else:  # follower
                if lane_offset == 0:
                    result = conn.vehicle.getFollower(veh_id, _MAX_DETECT_DIST)
                    if result is not None and result[0] != "":
                        return result
                    return None
                else:
    ...
    def _get_traffic_signal_obs(self, veh_id: str) -> np.ndarray:
        ...
        try:
            ...
            tls_list = conn.vehicle.getNextTLS(veh_id)
            if len(tls_list) == 0:
                return signal_obs
    ...
    def _get_obs(self) -> Dict:
        ...
        for i, cav_id in enumerate(cav_ids):
            try:
                ...
                x, y = self._conn.vehicle.getPosition(cav_id)
                positions[i] = (x, y)
                edges[i] = self._conn.vehicle.getRoadID(cav_id)
                lane_positions[i] = self._conn.vehicle.getLanePosition(cav_id)
    ...
    def _apply_actions(self, action: np.ndarray, cav_ids: List[str]):
        ...
        for i, cav_id in enumerate(cav_ids):
            if cav_id not in current:
                continue  # vehicle left between obs and action
            try:
                ...
                if self.cav_control_mode in {"hybrid", "longitudinal_only"}:
                    ...
                    current_speed = conn.vehicle.getSpeed(cav_id)
                ...
                if self.cav_control_mode in {"hybrid", "lane_change_only"}:
                    ...
                    lane_idx = conn.vehicle.getLaneIndex(cav_id)
    ...
    def _compute_reward(
        ...
        current = set(self._get_active_cavs())
        all_veh_ids = set(conn.vehicle.getIDList())
        ...
        for i, cav_id in enumerate(cav_ids):
            ...
            if exited_control and cav_id not in all_veh_ids:
                ...
                continue
            try:
                speed = conn.vehicle.getSpeed(cav_id)
                accel = conn.vehicle.getAcceleration(cav_id)
                curr_lane_idx = conn.vehicle.getLaneIndex(cav_id)

                # 1. Speed reward: r_speed = w · (v / v_max)
                speed_error = speed / _MAX_SPEED
                r_speed = _W_SMOOTH_SPEED * speed_error - 0.05

                # 2. Acceleration penalty: r_accel = -w · (a / a_max)^2
                r_accel = -_W_ACCEL_PENALTY * ((accel / _MAX_ACCEL) ** 2 if accel >= 0 else (accel / _MAX_DECEL) ** 2)
                ...
                leader = conn.vehicle.getLeader(cav_id, _MAX_DETECT_DIST)
                ttc_below_thresh = False
                if leader is not None and leader[0] != "":
                    leader_speed = conn.vehicle.getSpeed(leader[0])
