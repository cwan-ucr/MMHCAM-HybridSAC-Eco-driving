...
def _to_scalar(v):
	"""Convert Python numbers and 0-d tensors to a Python float.
	Returns None for anything that is not a scalar (e.g., multi-d tensors)."""
...
class _TeeStream:
	...
	def __init__(self, stream, filepath):
		...
		# Strip ANSI color codes when writing to file for readability
		self._ansi_re = re.compile(r'\x1b\[[0-9;]*m')

	def write(self, data):
		self.stream.write(data)
		self.file.write(self._ansi_re.sub('', data))
...
class Logger:
	...
	def __init__(self, cfg):
		...
		self._plot_skip_initial_episodes = int(cfg.get("plot_skip_initial_episodes", 0))
		self._group = cfg_to_group(cfg)
		self._seed = cfg.seed
		self._eval = []
		self._train_metrics = []

		# Redirect stdout/stderr to ALSO append to a log file in work_dir
		log_file = self._log_dir / "console.log"
	...
	def finish(self, agent=None):
		...
		# Auto-generate the training curves figure
		try:
			from common.plot_training import plot_training_curves
			plot_training_curves(
				self._log_dir,
				skip_initial_episodes=self._plot_skip_initial_episodes,
			)
		except Exception as e:
			print(colored(f"Failed to plot training curves: {e}", "red"))
		if self._wandb:
			self._wandb.finish()
	...
	def log(self, d, category="train"):
		...
		if category == "eval" and self._save_csv:
			...
			for k, v in d.items():
				...
				if s is not None:
					row[k] = s
			self._eval.append(row)
			pd.DataFrame(self._eval).to_csv(
				self._log_dir / "eval.csv", index=None
			)
		if category == "train" and self._save_csv:
			...
			for k, v in d.items():
				...
				if s is not None:
					row[k] = s
			self._train_metrics.append(row)
			pd.DataFrame(self._train_metrics).to_csv(
